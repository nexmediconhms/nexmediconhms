...
export async function loadClinicUser(): Promise<ClinicUser | null> {
  ...
  // Attempt 2: Try by email (handles case where auth_id was inserted wrong)
  if (authUser.email) {
    ...
    if (!emailError && emailData) {
      // Fix the auth_id mismatch silently — update it to the correct value
      // This uses a separate call; if it fails due to RLS, that's okay
      supabase
        .from('clinic_users')
