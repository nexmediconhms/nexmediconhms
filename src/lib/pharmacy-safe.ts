...
async function dispenseFallback(params: DispenseParams): Promise<DispenseResult> {
  ...
  // Single atomic UPDATE with WHERE current_stock >= quantity
  // If stock was already decremented by another request, this WHERE fails
  // and `count` will be 0 (no rows updated).
  const { data: updatedRows, error: updateErr } = await supabase
    .from('pharmacy_medicines')
    .update({
      current_stock: supabase.rpc ? undefined : undefined, // placeholder
  ...
  // Unfortunately, Supabase JS client doesn't support `SET current_stock = current_stock - N`
  // directly in .update(). So we use a raw RPC approach as second fallback.

  // Alternative: Use a raw SQL query via RPC
  try {
