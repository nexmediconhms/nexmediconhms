...
const FORMS = [
  {
    ...
    desc:     'Full name, DOB, gender, blood group, mobile, address, Aadhaar, emergency contact, insurance, consent',
...
export default function FormsPage() {
  ...
  function handleExtracted(formId: string, result: OCRResult) {
    // Use 'generic' key for registration form (no patientId yet)
    const key = formId === 'patient_registration' ? 'ocr_prefill_generic' : `ocr_prefill_${patientId || 'generic'}`
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-3 mt-4 flex-wrap">
                    ...
                    {/* Go to Patient Registration with prefill */}
                    {form.id === 'patient_registration' && scanDone[form.id] && (
                      <button onClick={() => router.push('/patients/new?prefill=1')}
                        className="flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-lg bg-indigo-700 text-white hover:bg-indigo-800 transition-colors">
