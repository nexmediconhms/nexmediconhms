...
import { logPeriod, updatePeriod, getPatientCycles, analyzeCycles } from '@/lib/menstrual-tracking';
import { saveScore, getPatientScores, calculateMRS, calculateBishopScore, calculateFGScore } from '@/lib/clinical-scores';
import { createWorkup, updateWorkupStep, getPatientWorkups } from '@/lib/infertility-wizard';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (tool === 'menstrual') {
      ...
      if (action === 'list') {
        const result = await getPatientCycles(supabase, data.patient_id, data.limit);
        if (result.error) return NextResponse.json({ error: result.error }, { status: 500 });
    ...
    if (tool === 'score') {
      if (action === 'calculate') {
        ...
        switch (data.score_type) {
          case 'mrs': result = calculateMRS(data.scores); break;
          case 'bishop': result = calculateBishopScore(data.scores); break;
          case 'ferriman_gallwey': result = calculateFGScore(data.scores); break;
          default: return NextResponse.json({ error: `Unknown score: ${data.score_type}` }, { status: 400 });
        }
        // Save to DB if patient_id provided
        if (data.patient_id) {
          await saveScore(supabase, data.patient_id, result, data.encounter_id, data.scored_by);
        }
        return NextResponse.json({ result });
      }
      if (action === 'history') {
        const result = await getPatientScores(supabase, data.patient_id, data.score_type);
        if (result.error) return NextResponse.json({ error: result.error }, { status: 500 });
    ...
    if (tool === 'infertility') {
      ...
      if (action === 'update_step') {
        const result = await updateWorkupStep(supabase, data.workup_id, data.step_key, data.step_data);
        if (result.error) return NextResponse.json({ error: result.error }, { status: 500 });
