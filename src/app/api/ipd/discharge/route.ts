...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 8. Audit log
    //
    // BIL-5 / AUD-2 fix (June 2026): route through the canonical
    ...
    // the verification chain noticing). The RPC contract is the same
    // shape as the audit() helper in @/lib/audit.
    try {
      const auditChanges = {
        discharge_date,
        discharge_time,
      ...
      if (auditErr) {
        ...
        // (the discharge already committed in step 7).
        console.warn(
