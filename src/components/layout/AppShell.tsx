 ...
 *
 * FIXES:
 * BUG 1: First-time admin bootstrap — when clinic_users is empty, show setup
 *   form DIRECTLY in AppShell instead of redirecting to /login (which causes
 *   infinite loops or the "Access Not Configured" error).
 *
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // ══════════════════════════════════════════════════════════════
  // FIRST-TIME SETUP SCREEN — shown when clinic_users table is empty
  // The first person to log in becomes the admin.
  // ══════════════════════════════════════════════════════════════
  if (showFirstTimeSetup) {
    async function handleFirstTimeSetup(e: React.FormEvent) {
      ...
      if (result.success) {
        setSetupDone(true)
        // Reload user after 1.5s to show the dashboard
        setTimeout(() => {
        ...
        setSetupError(result.error || 'Failed to create admin account. Check your Supabase configuration.')
      }
    }
            ...
            {setupDone ? (
              ...
              <>
                ...
                <p className="text-sm text-gray-500 mb-6">
                  No users exist yet. Enter your name to become the first admin.
                  You can add doctors and staff later from Settings.
                </p>
  ...
  // ══════════════════════════════════════════════════════════════
  // NO PROFILE SCREEN — user authenticated but no clinic_users row
  // Also offers to bootstrap if table is actually empty (fallback)
  // ══════════════════════════════════════════════════════════════
  if (noProfile) {
    async function handleEmergencyBootstrap() {
      // Check one more time if this is actually a first-time setup
      const firstTime = await isFirstTimeSetup()
      if (firstTime) {
        setNoProfile(false)
        setShowFirstTimeSetup(true)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white rounded-2xl shadow-lg max-w-md w-full p-8 text-center">
          ...
          {/* Emergency bootstrap button — if clinic_users is actually empty */}
