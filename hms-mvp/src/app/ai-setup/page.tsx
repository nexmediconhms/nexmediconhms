  ...
  return (
    <AppShell>
      <div className="p-6 max-w-2xl mx-auto">
              ...
              {result?.ok && (
                <>
                  ...
                  <div className="text-sm text-green-700 space-y-0.5">
                    {(result as any).anthropic?.ok && (
                      <p>✓ Anthropic Claude — {(result as any).anthropic.model} ({(result as any).anthropic.key_preview})</p>
                    )}
                    {(result as any).openai?.ok && (
                      <p>✓ OpenAI GPT-4o — {(result as any).openai.model} ({(result as any).openai.key_preview})</p>
                    )}
                    {!(result as any).anthropic?.ok && <p className="text-amber-700">⚠ Anthropic: {(result as any).anthropic?.error}</p>}
                    {!(result as any).openai?.ok && <p className="text-amber-700">⚠ OpenAI: {(result as any).openai?.error}</p>}
                  </div>
                </>
              )}
              {result && !result.ok && (
        ...
        {result && !result.ok && (result.step === 'key_placeholder' || result.step === 'key_missing' || result.step === 'no_keys') && (
